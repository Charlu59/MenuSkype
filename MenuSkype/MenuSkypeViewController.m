...
-(void)setMenuContent {
    ...
    for (int i=-1; i<nbItems; i++) {
        x+= MARGE;
        UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(x, 0, self.view.frame.size.width, MENU_HEIGHT)];
...
-(void)scrollViewDidScroll:(UIScrollView *)scrollView {
    if (scrollView == content) {
        ...
        UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, MENU_HEIGHT)];
        [label setFont:[UIFont systemFontOfSize:FONT_SIZE]];
